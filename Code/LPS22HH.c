...
static unsigned char write_reg(unsigned char devAddr,unsigned char reg,unsigned char *pData, unsigned char amount)
{
...
//	return 0;
	...
	if(i2c_mem_write(devAddr,reg,pData,amount))
	{
		return 1;
	}
	return 0;
}
static unsigned char read_reg(unsigned char devAddr,unsigned char reg,unsigned char *pData, unsigned char amount)
{
//	if(i2c_mem_read(devAddr, reg, pData, amount)!=I2C_OK) {
//		return 1;
//	}
//	return 0;
	if(i2c_mem_read(devAddr,reg,pData,amount))
	{
		return 1;
	}
	return 0;
...
unsigned char lps22hh_init(LPS22HH_t* obj)
{
	...
	if(read_reg(obj->devAddr, WHO_AM_I, cmd, 1))
	{
		return 1;
	}
	...
	lps22hh_reset(obj);
	cmd[0]=SET_IF_CTRL;
	if(write_reg(obj->devAddr,IF_CTRL, cmd, 1))//设置IF_CTRL
	{
		return 1;
	}
	...
	cmd[1]=SET_CTRL_REG2;
	cmd[2]=SET_CRTL_REG3;
	if(write_reg(obj->devAddr,CTRL_REGn,cmd,3))//设置CTRL_REGn
	{
		return 1;
	}
	for(;;) {
		if(read_reg(obj->devAddr,INT_SOURCE, cmd, 1))
		{
			return 1;
		}
...
unsigned char lps22hh_get_tmp(LPS22HH_t* obj,float *tmp)
{
	...
	unsigned int raw=0;
	if(read_reg(obj->devAddr,STATUS, &cmd, 1))
	{
		return 1;
	}
	...
	if(read_reg(obj->devAddr,TEMP_OUTn, buf, 2))
	{
		return 1;
	}
...
unsigned char lps22hh_get_prs(LPS22HH_t* obj,float *prs)
{
	...
	if(read_reg(obj->devAddr,PRESS_OUTn, buf, 3))
	{
		return 1;
	}
...
//	delayxms(10);
	if(read_reg(obj->devAddr,PRESS_OUTn, buf, 5))
	{
		return 1;
	}
