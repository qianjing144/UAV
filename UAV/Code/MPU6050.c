...
unsigned char mpu6050_init(MPU6050_t *obj)
{
	...
	cmd=0x01;
	write_reg(obj,PWR_MGMT,&cmd,1);
	cmd=0x09;
	write_reg(obj,SMPLRT_DIV,&cmd,1);
	cmd=0x06;
...
{
	unsigned char cmd;
	read_reg(obj, INT_STATUS, &cmd, 1);//清空中断标志位
	if((cmd&0x01)!=0x01)
	{
		return 1;
	}
	read_reg(obj, ACCEL_XOUT, pRaw, 6);
	return 0;
}
...
unsigned char getAllRaw(MPU6050_t *obj,unsigned char *pRaw) //14 A6 T2 G6
{
	...
	read_reg(obj, ACCEL_XOUT, pRaw, 14);
	return 0;
}
